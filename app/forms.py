...
class SignupForm(FlaskForm):
    username = StringField('Username', validators=[
        ...
        validators.Length(max=32, message='Username must be at most 32 characters.')
    ])
    password = PasswordField('Password', validators=[
        validators.DataRequired(),
        validators.Length(min=12, message='Password must be at least 12 characters.'),
        validators.Regexp(
            regex="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).*$",
    ...
    confirm_password = PasswordField('Confirm Password', validators=[
        validators.DataRequired(),
        validators.EqualTo('password', message='Passwords does not match. Try again.')
    ])
...
class EditPasswordForm(FlaskForm):
    ...
    confirm_new_password = PasswordField('Confirm New Password', validators=[
        validators.DataRequired(),
        validators.EqualTo('new_password', message='Passwords do not match. Try again.')
    ])
    submit = SubmitField('Update Password')

class EditFontSizeForm(FlaskForm):
    font_size_choices = [('normal', 'Normal'), ('small', 'Small'), ('large', 'Large')]
    font_size = SelectField('Font Size', choices=font_size_choices, validators=[DataRequired()])
    submit = SubmitField('Save Changes')
