...
def home():
    ...
    user_ids_to_query.append(current_user.id)  # Include the current user's posts
    all_posts = Post.query.filter(Post.user_id.in_(user_ids_to_query)).order_by(Post.date_posted.desc()).all()

    # Pass the liked status of each post to the template
    liked_posts = [post.has_user_liked(current_user) for post in all_posts]

    return render_template('home.html', posts=all_posts, liked_posts=liked_posts, search_form=search_form)
...
def explore():
    ...
    if search_form.validate_on_submit():
        ...
        users = User.query.filter(or_(User.username.ilike(search_query), User.id.ilike(search_query))).all()
    else:
        # Display all users initially
        users = User.query.all()

    return render_template('explore.html', users=users, search_form=search_form)
...
def post_create():
    if request.method == 'POST':
        ...
        image = request.files.get('image')  # Assuming 'image' is the name of the file input

        # Create a new post
        new_post = Post(content=content, user=current_user)  # Assuming you have a current_user object

        # Save the post to the database
        db.session.add(new_post)
        db.session.commit()

        # Redirect to the home page or the post detail page
        return redirect(url_for('home'))
...
def delete_post(post_id):
    ...
    if post_to_delete and post_to_delete.user == current_user:
        ...
        # If the post doesn't exist or the user is not the owner, abort with a 403 error
        abort(403)
...
@login_required
def settings():
    # Create a form for adjusting font size
...
def other_user_profile(username):
    ...
    if other_user:
        return render_template('other_user_profile.html', other_user=other_user, liked_posts=liked_posts, search_form=search_form)
    else:
        abort(404) 
